...
export const registerBeneficiary = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error registering beneficiary", error: error.message });
  }
};
...
export const getAllBeneficiaries = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Beneficiaries retrieved successfully", beneficiaries });
  } catch (error) {
    res
      .status(500)
      .json({
...
export const updateBeneficiary = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error updating beneficiary", error: error.message });
  }
};
...
export const deleteBeneficiary = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error deleting beneficiary", error: error.message });
  }
};
...
export const scanQRCode = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error scanning QR code", error: error.message });
  }
};

export const updateBeneficiaryStatus = async (req, res) => {
    try {
        const { uniqueId, status } = req.body; // Extract uniqueId and status from the request body
        ...
        // Find the beneficiary by uniqueId
        const beneficiary = await Beneficiary.findOne({ uniqueId });
        ...
        // Update the correct 'status' field (lowercase)
        beneficiary.status = status; // Update 'status' field
        await beneficiary.save();

        res.status(200).json({ message: 'Beneficiary status updated successfully', beneficiary });
    } catch (error) {
        res.status(500).json({ message: 'Error updating beneficiary status', error: error.message });
    }
};
...
export const getBeneficiaryById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching beneficiary", error: error.message });
  }
};
